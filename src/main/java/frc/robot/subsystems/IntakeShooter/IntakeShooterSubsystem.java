  ...
  public void intake() {
    switch (intakeState_) {
      case MoveToIntakeStart:
        ...
        if (Math.abs(
                io_.getTilt().getPosition().getValueAsDouble() * 360
                    - TiltConstants.upDownCanMoveIntakeTarget)
            < 2) {
      ...
      case MoveToIntake:
        ...
        if (Math.abs(
                    io_.getTilt().getPosition().getValueAsDouble() * 360
                        - TiltConstants.intakeTarget)
                < 3
            && Math.abs(
                    io_.getUpDown().getPosition().getValueAsDouble() - UpDownConstants.intakeTarget)
                < 3) {
      ...
      case HasNoteIdle:
        if (Math.abs(
                intakeFeederStartPosSeenNote_
                    + FeederConstants.keepSpinningIntakeTarget
                    - io_.getFeeder().getPosition().getValueAsDouble() * 360)
            < 2.0) {
          io_.stopFeeder();
        }
        break;
