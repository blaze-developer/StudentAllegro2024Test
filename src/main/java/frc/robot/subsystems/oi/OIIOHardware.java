...
public class OIIOHardware extends CommandGenericHID implements OIIO {
  ...
  public Trigger actionAmp() {
    return new Trigger(
        () ->
            !actionSpeaker().getAsBoolean()
                && !actionTrap()
                    .getAsBoolean()); // whenever its not flipped to left OR right :3 (in the
                                      // middle)
  }
  ...
  public Supplier<ActionType> actionTypeSupplier() {
    return () -> {
      if (actionSpeaker().getAsBoolean()) return ActionType.SPEAKER;

      if (actionTrap().getAsBoolean()) return ActionType.TRAP;
  ...
  public Supplier<ShootType> shootTypeSupplier() {
    return () -> {
      if (shootPodium().getAsBoolean()) return ShootType.PODIUM;

      if (shootSubwoofer().getAsBoolean()) return ShootType.SUBWOOFER;
