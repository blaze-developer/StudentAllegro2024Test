...
/**
 * OI layer interface for different hardware implementations to be used. When extending this class,
 * simply override all abstract methods to return a trigger to the desired button mapping.
 */
public interface OIIO {
  ...
  /**
   * The method to actually update the inputs to the correct values, must be called in the subsystem
   * periodic method for correct logging.
   *
  ...
  public default void updateInputs(OIIOInputsAutoLogged inputs) {
    ...
    // Other two not included here because they technically dont count as 'inputs' but 'outputs' in
    // the current model.
  }
   ...
   *
   * @return A supplier that gives the current state of the action type switch. (PODIUM, AUTO,
   *     SUBWOOFER)
   */
   ...
   *
   * @return A supplier that gives the current state of the shooting type switch. (SPEAKER, AMP,
   *     TRAP)
   */
